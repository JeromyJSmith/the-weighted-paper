...
const VARIANTS = {
  ...
  V4: { name: 'Nine-option zero-sum', nick: 'nine options', comp: '2 Core · 2 Bonus · 4 Poor · 1 Serious', max: 6, min: -6, pass: 4, headroom: 2,
    tests: 'Select-all scores exactly 0',
    fail: 'Scoring zero or below because everything sounded reasonable.',
    body: 'BACP’s own published shape. Positives total +6, negatives total −6, select-all scores exactly zero. The four Poor options fail in different ways: one defers, one over-investigates, one contacts the wrong party, one does the right thing in the wrong order.' },
